...
public class LuxsoftBoardHomeController {
    ...
    private void loadGauge() {
        ...
        speedGauge.setPrefWidth(70);
        speedGauge.setPrefHeight(130);
        innerGridPane.add(speedGauge, 0, 1);

        new Thread(() -> {
            while (running) {
                try (FileInputStream fis = new FileInputStream("src/luxsoftBoard/img/speed.txt");
                     BufferedReader reader = new BufferedReader(new InputStreamReader(fis))) {

                String speedLine = reader.readLine();
                if (speedLine != null) {
                    double speed = Double.parseDouble(speedLine.trim());  // Use Double.parseDouble here
                    Platform.runLater(() -> {
                        speedGauge.setValue(speed);
                        double prevSpeed = previousSpeed.get();
                        if (speed > 60 && prevSpeed <= 60) {
                            playSound("src/luxsoftboard/img/beepSound.wav");
                        } else if (speed <= 60 && prevSpeed > 60) {
                            stopSound();
                        }
                        previousSpeed.set((int) speed);  // Update AtomicInteger with the integer part
                    });
                }
